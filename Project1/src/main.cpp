...
void question_b_i(const std::map<int, std::vector<int>>& dataSets)
{
    ...
    writeOutput1("output1.csv", x_inputSize, y_KeyComparisons, timeTaken);    
}
...
void question_b_ii(const std::map<int, std::vector<int>>& dataSets)
{
    ...
    writeOutput2("output2.csv", x_thresholdval, y_KeyComparisons, timeTaken);    
}
...
int main()
{
    ...
    for (const auto& dataSize : dataSizes)
        dataSets[dataSize] = generateDataSet(dataSize, 100);
